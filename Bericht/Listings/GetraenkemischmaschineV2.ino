/*Programm fuer Softwareprojekt "Getraenkemischmaschine"
   DHBW Karlsruhe TMT20B1 (Goetz, Hoeckele, Lobert)
...
int S;                //Welcher pwm wert wird veraendert
...
int Schalter = 13;  //Fuellen Druckknopf
...
NexDSButton bt0 = NexDSButton(3, 4, "bt0");   //DSButton zum Spuelen
NexDSButton bt1 = NexDSButton(3, 5, "bt1");   //DSButton zum Spuelen
NexDSButton bt2 = NexDSButton(3, 6, "bt2");   //DSButton zum Spuelen
NexDSButton bt3 = NexDSButton(3, 14, "bt3");  //DSButton zum Spuelen
NexDSButton bt4 = NexDSButton(3, 15, "bt4");  //DSButton zum Spuelen
...
void resPopCallback(void *ptr)  //reset release -> ruecksetzen auf pwm=0
{
  pwm1 = 0;
  pwm2 = 0;
  pwm3 = 0;
...
void b3PopCallback(void *ptr)  //Save release
{
  ...
  for (int i = 0; i < 10; i++) {  //Sicherstellen das Wert richtig uebernommen wurde
    if (sl == 0) {
      h0.getValue(&sl);
    }
  }
  if (sl != 0) {
    if (S == 1) {  //Feststellen welcher in welche PWM der Sliderwert uebernommen werden soll
      //pwm1 = 125 + ((sl/100) * 130);
...
void setup() {
  S = 0;                     //kein pwm Wert auswaehlen
  pinMode(Schalter, INPUT);  //Eingaenge und Ausgaenge definieren
...
void loop() {
  nexLoop(nex_listen_list);           //Nextion events ueberpruefen
  if (digitalRead(Schalter) == HIGH)  //Fuellen Knopf an -> PWM Werte an Pumpen geben
  {
  ...
  } else //Fuellen Knopf aus -> Pumpen aus
  {
